...
# test_slice = c("1", "2", "3", "4", "5")
# thesample2 <- as.data.table(thesample) %>% filter(Position == "Orig", slice %in% test_slice)  %>% dplyr::select(rid_level, swi_slope, tpi, twi, valley_depth_2)
# rnge <- range(map$rid_level)
# 
# hypervars = c('rid_level', 'swi_slope', 'tpi', 'twi', 'valley_depth_2')
# ###testingt
# result <- compare_hypercubes(target_hypercube = map, sample_hypercube = thesample2, bins = 10, varlist = hypervars)
