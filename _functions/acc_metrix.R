...
acc_metrix <- function(data, theta = 0.5) {
  ...
  ### Calculate the fuzzy value for target and predicted primary call
    data1 <- left_join(data, fMat, by = c("target" = "target", ".pred_class" = "Pred")) %>% 
      mutate_if(is.character, as.factor) %>% 
      mutate(fVal = replace_na(fVal,0)) %>% 
      mutate(p_fval = fVal) %>% 
      dplyr::select(-fVal)
    
    ### Calculate the fuzzy value for target and predicted Alternate call
    ...
    totals <- full_join(tot_trans , tot_pred, by = c("target" = ".pred_class")) %>%
      mutate(pred.tot = replace_na(pred.tot ,0))
    
    data <- left_join(data, totals, by = "target") %>% ungroup
    
    # check if all map units predicted?
    ...
    # TO follow up with WILL's script???
    # I dont quite get what is going on here?  Why was it only add.pred.lev1 here and not 2?
    # I adjusted this to check for both predicted and missing and missing and predicted
    
    # add.pred.lev1 <- anti_join( pred.lev, targ.lev,  by = "levels")
    # add.pred.lev2 <- anti_join( targ.lev, pred.lev, by = "levels")
    # 
    # add.pred.lev <- rbind(add.pred.lev1, add.pred.lev2)
    # 
    # 
    # data$.pred_class %in% add.pred.lev
    
    ## what is the point of this? I dont think this is doing the intended? rowwise?
    ## Needs fixing - i am leaving this out for the minute
    # 
    #     data <- data %>%
    #       rowwise()%>%
    #       mutate(target.new = ifelse(.pred_class %in%
    #                                    add.pred.lev, as.character(.pred_class), as.character(target)),
    #                             trans.tot.new = ifelse(.pred_class %in%
    #                                                      add.pred.lev, 0, trans.tot)) %>%
    #       mutate_if(is.character, as.factor) %>%
  ...
  # 1) Machine learning metrics
    acc <- data %>% accuracy(target, .pred_class, na_rm = TRUE) %>%  
          dplyr::select(.estimate) %>% round(3) %>% pull()
  ...
  # 2) Spatial Statistics
    ...
    spatial_acc <- data %>% mutate(trans.sum = n(), acc = acc, mcc = mcc, kap = kap ) %>%
     ...
     # dplyr::mutate(trans.tot = n()) %>%  # already calculatd above
    ...
    # 3) aspatial metrics (overall and mapunit % correct)
    ...
    aspatial_acc <- left_join(aspatial_target, aspatial_pred, by = c("target" = ".pred_class")) %>% 
      dplyr::select(target, trans.tot, pred.tot) %>% 
      dplyr::mutate(trans.sum = sum(trans.tot)) %>% 
      mutate(no.classes = length(unique(target))) %>% 
    ...
    ## To Fix 
    ## SOMETHING WRONG HERE _ IS THIS JUST SUPPOSED TO BE PA and not including primary? 
    ##Seems like problem as holding NAs?
    
    
    #  This is resulting in blank target values
    data_pa <- data %>% mutate_if(is.factor, as.character) %>% 
      mutate(target = ifelse(target2 == .pred_class, as.character(target2), as.character(target))) %>% 
      mutate_if(is.character, factor)
    
    aspatial_target <- data_pa  %>% dplyr::select(target) %>%  
      dplyr::add_count(target, name = 'trans.tot') %>% distinct() 
    
    aspatial_pred <- data_pa  %>% dplyr::select(.pred_class) %>%
      dplyr::add_count(.pred_class, name = 'pred.tot') %>%
      distinct()  
    
    # NOTE THIS IS ALSO DROPPING ROWS??? in this Left join?
    ...
    aspatial_acc_pa <- left_join(aspatial_target, aspatial_pred,  by = c("target" = ".pred_class")) %>% 
      ...
      dplyr::mutate(across(where(is.numeric), ~ replace(.,is.nan(.),0))) %>% 
      ungroup() %>%
      mutate(aspat_pa_theta0 = sum(aspat_pa_wtd),
      ...
      ungroup() %>% distinct() %>% dplyr::select(-pred.tot)
