...
balance_optimisation_raw <- function(trDat , extrarun = FALSE, extradat = NULL, downsample = FALSE, smote = FALSE, use.neighbours = TRUE){
  ...
  sresults <- foreach(k = levels(slices)) %do% {
    ...
    BGC_train <- trDat %>% dplyr::filter(!slice %in% k, position %in% "Orig") %>%
      filter(is.na(target2) | target2 == "") %>% dplyr::select(-slice, -bgc, -position, -target2, -id) %>%  # -transect_id,  , -id
      mutate(across("tid", str_replace, " ", "")) %>%
      mutate(across("tid", str_replace, " ", ""))%>% mutate(target = factor(target))#%>% select(-target2)
    ...
    # check if enough data for each class to smote
      ...
      pull(target)
       ...
       BGC_train <- BGC_train %>% data.frame %>% mutate(target = as.factor(target)) %>%  drop_na(target) %>% droplevels()
    ...
    BGC_test <- BGC_test %>%
      dplyr::select(-slice,-target2, -position, -bgc) %>% mutate(target = as.factor(target))#-transect_id,
    #
