...
build_model_raw <- function(trDat , extrarun = FALSE, extradat = NULL, downsample = FALSE, smote = FALSE, use.neighbours = TRUE){
  ...
  sresults <- foreach(k = levels(slices)) %do% {
    ...
    BGC_train <- trDat %>% dplyr::filter(!slice %in% k, position %in% "Orig") %>%
      filter(is.na(mapunit2) | mapunit2 == "") %>% dplyr::select(-slice, -bgc, -position, -mapunit2, -id) %>%  # -transect_id,  , -id
      mutate(across("tid", str_replace, " ", "")) %>%
      mutate(across("tid", str_replace, " ", ""))%>% mutate(mapunit1 = factor(mapunit1))#%>% select(-mapunit2)
    ...
    # check if enough data for each class to smote
       MU_count <- BGC_train %>% dplyr::count(mapunit1) %>% filter(n < 10) %>%
      pull(mapunit1)


       BGC_train <- BGC_train %>% data.frame %>% mutate(mapunit1 = as.factor(mapunit1)) %>%  drop_na(mapunit1) %>% droplevels()
    ...
    BGC_test_all <- BGC_test # keep for the mapunit2 alt call.
    BGC_test_transect_no <- length(unique(BGC_test_all$transect_id))
    BGC_test <- BGC_test %>%
      dplyr::select(-slice,-mapunit2, -position, -bgc) %>% mutate(mapunit1 = as.factor(mapunit1))#-transect_id,
    #
    null_recipe <-  recipe(mapunit1 ~ ., data = BGC_train) %>%
      update_role(tid, new_role = "id variable")


    randf_spec <- rand_forest(trees = 151) %>%# mtry = mtry, min_n = min_n,
    ...
    ######### Predict Test
    test_target <- BGC_test_all %>% dplyr::select(id,mapunit1, mapunit2)
...
### THIS NEEDS TO BE PULLED INTO A FUNCTION OPTION
    ...
    test.pred <- test.pred %>% mutate(mapunit1 = ifelse(mapunit1 %in% convert, "nonfor", mapunit1),
                                      mapunit2 = ifelse(mapunit2 %in% convert, "nonfor", mapunit2) ,
                                      .pred_class = ifelse(.pred_class  %in% convert, "nonfor", .pred_class ))

    test.pred <- test.pred %>%  mutate_all(as.factor) %>% distinct()
    ###harmonize levels
    targ.lev <- levels(test.pred$mapunit1)
 ...
 #            bgc = bgc.label)
    ...
    acc.compare <- acc.compare  %>%
      dplyr::select(bgc, slice, mapunit1,balance, trans.sum, trans.tot, pred.tot, acc, kap, everything() )
    ...
    for(i in smote_iterations){
      ...
      sresults <- foreach(k = levels(slices)) %do% {
       ...
       #print(k)
        ...
        BGC_train <- trDat %>% dplyr::filter(!slice %in% k, position %in% "Orig") %>%
          filter(is.na(mapunit2) | mapunit2 == "") %>% dplyr::select(-slice, -bgc,  -transect_id, -position, -id) %>%
          mutate(across("tid", str_replace, " ", "")) %>%
          mutate(across("tid", str_replace, " ", ""))
        ...
        MU_count <- BGC_train %>% dplyr::count(mapunit1) %>% filter(n < 10) %>%
          pull(mapunit1) %>% droplevels()

        if(length(MU_count) > 0) {
          print("remove classes with very low training points from smoting test")
          BGC_train <- BGC_train %>%
            filter(!mapunit1 %in% MU_count) %>% droplevels()
        }
        #


        BGC_train <- BGC_train %>%
          dplyr::select(-mapunit2) %>%  drop_na() %>% mutate(mapunit1 = factor(mapunit1), tid = as.character(tid)) %>%  droplevels()
        ...
        BGC_test_transect_no <- length(unique(BGC_test_all$transect_id))
        BGC_test <- BGC_test %>%
          dplyr::select(-slice,-mapunit2,-transect_id, -bgc)
        #
      ...
      ###_______________________________________________________
       ...
       ###harmonize factor levels
        targ.lev <- levels(test.pred$mapunit1)
        targ2.lev <- levels(test.pred$mapunit2)
        pred.lev <- levels(test.pred$.pred_class)
        levs <- c(targ.lev,targ2.lev, pred.lev) %>% unique()
        test.pred$mapunit1 <- factor(test.pred$mapunit1, levels = levs)
        test.pred$mapunit2 <- factor(test.pred$mapunit2, levels = levs)
        test.pred$.pred_class <- factor(test.pred$.pred_class, levels = levs)
        # output test predictions
